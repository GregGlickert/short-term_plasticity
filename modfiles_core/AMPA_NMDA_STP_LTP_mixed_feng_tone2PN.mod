...
NEURON {
    ...
    : short term faciliation and depression by Greg Glickert
...
BREAKPOINT {
    ...
    : AMPA Receptor
    g_AMPA = (1e-3)*gmax_AMPA*(B_AMPA - A_AMPA)*facfactor
    i_AMPA = g_AMPA*(v-E_AMPA) * W
    : NMDA Receptor
    mggate = 1 / (1 + exp(-slope_NMDA*v) * (mgo_NMDA/scale_NMDA))
    g_NMDA = (1e-3)*gmax_NMDA*mggate*(B_NMDA - A_NMDA)*facfactor
